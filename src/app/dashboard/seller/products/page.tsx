...
export default function SellerProductsPage() {
  ...
  const handleAddToCart = (product: Product) => {
    ...
    // Add with default base unit and quantity 1
    setCart([...cart, { 
      product, 
      quantity: '1', 
  ...
  const handleCartQtyChange = (productId: number, qtyString: string) => {
    // Basic formatting constraint (allow empty, decimals, positive values)
  ...
  // Helper to validate and calculate cart item totals
  const getCartItemBreakdown = (item: CartItem) => {
    ...
    try {
      if (qtyStr !== '' && parseFloat(qtyStr) > 0) {
        // Calculate converted base quantity
  ...
  const handlePlaceOrder = async () => {
    ...
    // Filter out items with empty/invalid quantities
                  ...
                  <div key={item.product.id} className="glass-panel" style={{ padding: '1rem', background: 'rgba(7, 11, 19, 0.4)', borderColor: hasError ? 'rgba(239, 68, 68, 0.3)' : 'var(--card-border)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                      <div>
                        ...
                        <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Base: {formatINR(item.product.base_price_inr)} / {item.product.base_unit}</span>
                      </div>
                      <button 
